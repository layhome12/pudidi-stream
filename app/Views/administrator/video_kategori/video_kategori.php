...
<div class="modal fade" id="modal" tabindex="-1" role="dialog" aria-labelledby="Modal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <form action="<?= base_admin('video/video_kategori' . '_save') ?>" method="post" class="form-full" id="form-data" onsubmit="return false" enctype="multipart/form-data">
...
<script>
    var datatables = $('.datatables').DataTable({
        'ajax': {
            'url': '<?php echo base_admin('video/video_kategori' . '_fetch') ?>',
            'dataSrc': 'data',
            'type': 'POST',
            'data': function(form) {
                form.search = {
    ...
    function dt_form(t) {
        $.LoadingOverlay('show');
        $.post('<?= base_admin('/video/video_kategori' . '_form') ?>', {
    ...
    function dt_del(t) {
        Swal.fire({
            ...
            if (result.value) {
                $.post('<?= base_admin('/video/video_kategori' . '_del') ?>', {
                    'kvid': t.getAttribute('target-id')
                }, function(result, textStatus, xhr) {
                    $.LoadingOverlay('hide');
                    if (result.status > 0) {
    ...
    function dt_vid(t) {
        var link = '<?= base_admin('video/video_list') ?>' + '/' + t.getAttribute('target-id');
